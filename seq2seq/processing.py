...
from seq2seq.tokens import EOS_token
...
def tensorFromSentence(lang: Lang, sentence: str, device: torch.device):
    indexes = indexesFromSentence(lang, sentence)
    indexes.append(EOS_token)
    return torch.tensor(indexes, dtype=torch.long, device=device).view(-1, 1)


def tensorsFromPair(pair, input_lang, output_lang, device: torch.device):
